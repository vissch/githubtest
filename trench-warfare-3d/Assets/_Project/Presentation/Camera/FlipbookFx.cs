    ...
    public sealed class FlipbookFx
    {
        ...
        public FlipbookFx()
        {
            ...
            for (int k = 0; k < Sheets.Length; k++)
            {
                ...
                m.SetFloat("_Lit", s.Additive ? 0f : 1f);
        ...
        /// </summary>
        public void Add(Book book, Vector3 at, float width, float life, Kind kind = Kind.None, Vector3 velocity = default, float grow = 0f, float roll = 0f, float alpha = 1f, float glow = 1f, float height = 0f, float pop = 0f)
        {
            if (!Ready) return;
            if (cards.Count >= MaxCards) cards.RemoveAt(0);
            float h = height > 0f ? height : width / Mathf.Max(0.05f, aspect[(int)book]);
            cards.Add(new Card { Pos = at, Vel = velocity, Born = Time.time, Life = Mathf.Max(0.02f, life), Width = width, Height = h, Grow = grow, Roll = roll, Alpha = alpha, Glow = glow, Pop = pop, Book = book, Kind = kind });
        }
        ...
        public void Draw(float now, Bounds bounds)
        {
            ...
            for (int b = 0; b < (int)Book.Count; b++)
            {
                ...
                {
                    var c = cards[i]; if ((int)c.Book != b) continue;
                    ...
                    float fade = c.Alpha * (1f - Mathf.SmoothStep(0f, 1f, (k - 0.65f) / 0.35f));
                    float frame = (c.Kind & Kind.HoldLast) != 0 ? Mathf.Min(k * frames, frames - 1f) : Mathf.Min(k * frames, frames - 1.001f);
                    float bright = 1f + (c.Glow - 1f) * (1f - Mathf.Clamp01(k / 0.15f));   // the fire is out in the first sixth
                    batch[n++] = Pack(c.Pos, c.Width * swell, c.Height * swell, frame, fade, bright, c.Roll, c.Kind);
        ...
        public static Matrix4x4 Pack(Vector3 at, float width, float height, float frame, float alpha, float bright, float roll, Kind kind)
        {
            var m = Matrix4x4.identity;
            m.m03 = at.x; m.m13 = at.y; m.m23 = at.z;
            m.m00 = width; m.m11 = height; m.m22 = frame;
            m.m01 = alpha; m.m10 = bright; m.m02 = roll;
            m.m12 = (kind & Kind.Upright) != 0 ? 1f : 0f;
            m.m20 = (kind & Kind.Anchored) != 0 ? 1f : 0f;
            m.m21 = (kind & Kind.Mirror) != 0 ? -1f : 1f;
            return m;
        }
