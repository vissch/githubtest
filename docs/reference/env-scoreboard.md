...
| 2 | 2026-09-24 09:05 | T1 | NightMud | the instrument written in round 1 was itself blind to the men: `Graphics.RenderMeshIndirect` (4 sites in `VATRenderer`, 1 in `DebrisRenderer`) was never in my enumeration, so the counter would have reported a confident number with the infantry missing from it | `FrameBudget` landed for 6 files, plus `DrawIndirect` and a documented `Vertices` that excludes indirect submissions | none scored | EditMode 296/297, PlayMode 15/15. The one red (`ShaderInclusionTests`, `Unlit/Transparent`) comes from `UI/Selection/SelectionMarkers.cs`, which is UNTRACKED - a clean checkout does not contain it and is green | `TankRenderer`/`VATRenderer` (claude-14's resize in flight) and `BattlefieldProps`/`PropDestruction` (the PropWear session's) are routed in the working tree but NOT committed, so those 4 submitters are uncounted until their owners land. Still no capture, so T1 NightMud is still unscored |
| 3 | 2026-09-24 14:10 | T1/T3 | Winter | the level could not be reached from the game at all, and once reached, every light on it was the SAME BLUE - key (0.78,0.86,1.00), ambient, shade, mist, haze, bank - so no surface could differ in hue from any other: saturation p5 0.214 to p95 0.279, a range of 0.065 over 102,480 samples | the winter level made launchable (Ground enum, MatchLaunch.Field, BiomeProfile.ForGround, a mission card); warm key against a blue fill; snow keeps the ground's relief under it; the ink pass stops outlining terrain facets up close; a ground micro-kit (drift, broken crust, frozen tuft, clod); icicles on the houses; tracks take in snow and keep; a crater is bare earth that fills back in | none scored - T1 was never scored, so these are measurements, not score moves | EditMode 308/309, PlayMode 15/15, all six WinterLevelTests pass (run by claude-6f at 14:07 with this work in the tree; the one red is their SelectionMarkers shader fallback) | `Toon_URP.shader` is NOT in this commit - it carries another session's uncommitted crustRelief hoist that the snow-relief change depends on. `waterRenderers=0` still: W2/W10 ice has no surface on this level. Nothing walks on the map, so tracks are unverified in play |
